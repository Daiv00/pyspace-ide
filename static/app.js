...
async function localShare(){try{const d=await api('/api/local-share',{method:'POST'});const lan=d.urls?.[0]||d.cloud_url;modal(`<div class="share-modal"><div class="eyebrow">WI-FI FILE DROP</div><h2>Локальное хранилище</h2><p class="muted">Запусти PySpace на компьютере, подключённом к Wi‑Fi. Другой человек открывает ссылку или сканирует QR-код и может отправлять файлы и текст. Файлы остаются на этом компьютере после отключения телефона.</p><div class="share-link">${esc(lan)}</div><img class="qr" src="/api/local-share/${d.token}/qr" alt="QR"><div class="share-urls">${(d.urls||[]).map(x=>`<div>${esc(x)}</div>`).join('')}</div><p class="muted small">Важно: для настоящего Wi‑Fi-хранилища PySpace должен быть запущен локально на устройстве-хосте (0.0.0.0:${d.port||8080}). На Render эта функция не превращает Render в ваш домашний сервер.</p><button class="primary" onclick="copyText('${escAttr(lan)}')">Копировать ссылку</button></div>`)}catch(e){alert(e.message)}}
...
function modal(x){$('body').insertAdjacentHTML('beforeend','');$('body').style.overflow='hidden';$('body').classList.add('modal-open');$('body').dataset.modal=x;$('modal').classList.remove('hidden');$('body').querySelector('#body').innerHTML=x}function hide(){$('modal').classList.add('hidden');$('body').style.overflow='';$('body').classList.remove('modal-open')}
