import os,re,sqlite3,secrets,socket,subprocess,sys,tempfile,shutil
from pathlib import Path
from functools import wraps
from flask import Flask,request,jsonify,session,render_template
from werkzeug.security import generate_password_hash,check_password_hash
BASE=Path(__file__).resolve().parent; DB=BASE/'data/pyspace.db'; STORAGE=BASE/'storage'; PORT=int(os.getenv('PORT','8080')); HOST='0.0.0.0'; TIMEOUT=int(os.getenv('PYSPACE_RUN_TIMEOUT','8'))
app=Flask(__name__); app.secret_key=os.getenv('PYSPACE_SECRET',secrets.token_hex(32)); app.config['MAX_CONTENT_LENGTH']=10*1024*1024

def db():
 c=sqlite3.connect(DB); c.row_factory=sqlite3.Row; c.execute('PRAGMA foreign_keys=ON'); return c

def init_db():
 (BASE/'data').mkdir(exist_ok=True); STORAGE.mkdir(exist_ok=True); c=db(); c.executescript('''CREATE TABLE IF NOT EXISTS users(id INTEGER PRIMARY KEY AUTOINCREMENT,username TEXT UNIQUE NOT NULL,password_hash TEXT NOT NULL,role TEXT NOT NULL DEFAULT 'user',created_at TEXT DEFAULT CURRENT_TIMESTAMP);CREATE TABLE IF NOT EXISTS projects(id INTEGER PRIMARY KEY AUTOINCREMENT,owner_id INTEGER NOT NULL,name TEXT NOT NULL,created_at TEXT DEFAULT CURRENT_TIMESTAMP,FOREIGN KEY(owner_id) REFERENCES users(id) ON DELETE CASCADE);CREATE TABLE IF NOT EXISTS members(project_id INTEGER,user_id INTEGER,role TEXT NOT NULL DEFAULT 'editor',PRIMARY KEY(project_id,user_id),FOREIGN KEY(project_id) REFERENCES projects(id) ON DELETE CASCADE,FOREIGN KEY(user_id) REFERENCES users(id) ON DELETE CASCADE);''');
...
def register():
 ...
 c=db()
 try:
  role='admin' if c.execute('SELECT COUNT(*) n FROM users').fetchone()['n']==0 else 'user';c.execute('INSERT INTO users(username,password_hash,role) VALUES(?,?,?)',(n,generate_password_hash(pw),role));c.commit()
 except sqlite3.IntegrityError:c.close();return jsonify(error='Пользователь уже существует'),409
...
def create_project():
 ...
 c=db();cur=c.execute('INSERT INTO projects(owner_id,name) VALUES(?,?)',(user()['id'],n));pid=cur.lastrowid;c.execute('INSERT INTO members VALUES(?,?,?)',(pid,user()['id'],'owner'));c.commit();c.close();pdir(pid).mkdir(parents=True);f=fpath(pid,'main.py');f.write_text("print('Hello from PySpace!')",encoding='utf-8');return jsonify(id=pid,name=n)
...
def files(pid):
 ...
 b=pdir(pid);b.mkdir(parents=True,exist_ok=True);return jsonify([{'path':x.relative_to(b).as_posix(),'size':x.stat().st_size} for x in sorted(b.rglob('*')) if x.is_file()])
...
def getfile(pid,path):
 ...
 return jsonify(path=clean(path),content=x.read_text(encoding='utf-8',errors='replace'))
...
def server():
 ...
 return jsonify(host=ip,port=PORT,url=f'http://{ip}:{PORT}')
@app.post('/api/run')
@auth
def run():
 ...
 with tempfile.TemporaryDirectory(prefix='pyspace_') as td:
  ...
  try:r=subprocess.run([sys.executable,'-I',str(script)],cwd=work,capture_output=True,text=True,timeout=TIMEOUT,env=env);return jsonify(ok=r.returncode==0,returncode=r.returncode,output=(r.stdout+r.stderr)[-16000:])
...
# Initialize DB/storage for both direct execution and Gunicorn.
init_db()
