import os,re,sqlite3,secrets,socket,string,subprocess,sys,tempfile,shutil,zipfile,io,datetime
    ...
    if env_user and env_pw:
        ...
        if not existing:
            c.execute('INSERT INTO users(username,password_hash,role) VALUES(?,?,?)',
                     (env_user,generate_password_hash(env_pw),'admin'))
...
def pdir(pid):
    return STORAGE/f'project_{pid}'

def fpath(pid,p):
    p=clean(p)
    base=pdir(pid).resolve()
    x=(base/p).resolve()
    if base!=x and base not in x.parents:
        raise ValueError('Недопустимый путь')
    return x
...
def lan_ips():
    ...
    try:
        for x in socket.gethostbyname_ex(socket.gethostname())[2]:
            if x.startswith(('10.','192.168.','172.16.','172.17.','172.18.','172.19.','172.2','172.3')) and x not in ips:
                ips.append(x)
    except Exception:
        pass
    try:
        ...
        x=s.getsockname()[0]
        s.close()
        if x not in ips and x not in ('127.0.0.1','0.0.0.0'):
            ips.insert(0,x)
    except Exception:
        pass
...
def share_dir(token):
    if not re.fullmatch(r'[A-Za-z0-9_-]{7,80}',token):
        raise ValueError('bad token')
    return LOCAL_HUB/f'share_{token}'

def share_row(token):
    c=db()
    r=c.execute('SELECT * FROM local_shares WHERE token=? AND active=1',(token,)).fetchone()
    c.close()
    return r

def share_path(token,name):
    name=clean(name)
    base=share_dir(token).resolve()
    x=(base/name).resolve()
    if base!=x and base not in x.parents:
        raise ValueError('Недопустимый путь')
    return x
...
def received_file_row(fid):
    c=db()
    r=c.execute('''
        SELECT rf.*,u.username owner_name,ru.username recipient_name
        FROM received_files rf
        JOIN users u ON u.id=rf.owner_id
        LEFT JOIN users ru ON ru.id=rf.recipient_id
        WHERE rf.id=?
    ''',(fid,)).fetchone()
    c.close()
    return r
...
def register():
    ...
    n=str(d.get('username','')).strip()
    pw=str(d.get('password',''))
    if not re.fullmatch(r'[A-Za-z0-9_.-]{3,32}',n):
        return jsonify(error='Логин 3–32 символа'),400
    if len(pw)<8:
    ...
    try:
        ...
        c.execute('INSERT INTO users(username,password_hash,role) VALUES(?,?,?)',
                 (n,generate_password_hash(pw),role))
        c.commit()
...
def create_project():
    ...
    c=db()
    cur=c.execute('INSERT INTO projects(owner_id,name) VALUES(?,?)',(user()['id'],n))
    pid=cur.lastrowid
    c.execute('INSERT INTO members VALUES(?,?,?)',(pid,user()['id'],'owner'))
    c.commit()
    c.close()
    pdir(pid).mkdir(parents=True)
    fpath(pid,'main.py').write_text("print('Hello from PySpace!')",encoding='utf-8')
...
def rename_project(pid):
    ...
    if not n or len(n)>80:
        return jsonify(error='Некорректное имя'),400
    c=db()
    exists=c.execute('SELECT 1 FROM projects WHERE owner_id=? AND name=? AND id<>?',
                    (user()['id'],n,pid)).fetchone()
    if exists:
        c.close()
        return jsonify(error='Проект с таким именем уже существует'),409
    c.execute('UPDATE projects SET name=? WHERE id=?',(n,pid))
    c.commit()
    c.close()
...
def getfile(pid,path):
    ...
    return jsonify(
        path=clean(path),
        content=x.read_text(encoding='utf-8',errors='replace'),
        language=language_for(x.name)
    )
...
@auth
def savefile(pid):
    if not access(pid,True):
        return jsonify(error='Нет прав'),403
    d=request.get_json() or {}
    content=str(d.get('content',''))
    try:
        x=fpath(pid,d.get('path'))
    except ValueError as e:
        return jsonify(error=str(e)),400
...
def upload_files(pid):
    ...
    for f in incoming:
        ...
        x.parent.mkdir(parents=True,exist_ok=True)
        f.save(x)
        saved.append(x.relative_to(pdir(pid)).as_posix())
    ...
    with zipfile.ZipFile(mem,'w',zipfile.ZIP_DEFLATED) as z:
        if b.exists():
            for x in b.rglob('*'):
                ...
                z.write(x,rel)
    mem.seek(0)
    return send_file(mem,as_attachment=True,download_name=f'pyspace_project_{pid}.zip',mimetype='application/zip')
...
def delfile(pid):
    if not access(pid,True):
        return jsonify(error='Нет прав'),403
    try:
        x=fpath(pid,(request.get_json() or {}).get('path'))
    except ValueError as e:
        return jsonify(error=str(e)),400
    if x.exists():
        x.unlink()
...
def rename(pid):
    ...
    d=request.get_json() or {}
    try:
        a=fpath(pid,d.get('old_path'))
        b=fpath(pid,d.get('new_path'))
    except ValueError as e:
        return jsonify(error=str(e)),400
    if not a.exists():
        return jsonify(error='Файл не найден'),404
...
def share(pid):
    ...
    if role not in ('editor','viewer'):
        c.close()
        return jsonify(error='Роль editor/viewer'),400
    c.execute('INSERT INTO members VALUES(?,?,?) ON CONFLICT(project_id,user_id) DO UPDATE SET role=excluded.role',
             (pid,u['id'],role))
    c.commit()
    c.close()
    return jsonify(ok=True)
...
def create_local_share():
    ...
    while True:
        ...
        if not exists:
            c.execute('INSERT INTO local_shares(owner_id,token) VALUES(?,?)',(user()['id'],token))
            c.commit()
            c.close()
            break
        c.close()
...
def my_shares():
    c=db()
    rows=c.execute('SELECT token,created_at,active FROM local_shares WHERE owner_id=? ORDER BY id DESC',
                  (user()['id'],)).fetchall()
    c.close()
    result=[]
    for r in rows:
        b=share_dir(r['token'])
        total=sum(x.stat().st_size for x in b.rglob('*') if x.is_file()) if b.exists() else 0
        result.append({
            'token':r['token'],
...
def local_share_info(token):
    ...
    for x in sorted(b.rglob('*')):
        if x.is_file():
            items.append({'path':x.relative_to(b).as_posix(),'size':x.stat().st_size})
...
def upload_project_zip(project_id):
    ...
    try:
        f.save(str(tmp))
        files=safe_extract_zip(tmp,p)
        return jsonify(ok=True,project_id=project_id,files=files,count=len(files),
...
def assign_received_file(fid):
    ...
    c=db()
    f=c.execute('SELECT id FROM received_files WHERE id=?',(fid,)).fetchone()
    if not f:
        c.close()
        return jsonify(error='Файл не найден'),404
    ...
    if not ru:
        c.close()
        return jsonify(error='Пользователь не найден'),404
    c.execute('UPDATE received_files SET recipient_id=? WHERE id=?',(ru['id'],fid))
    c.commit()
    c.close()
    return jsonify(ok=True,username=ru['username'])
...
def delete_received_file(fid):
    ...
    if not f:
        return jsonify(error='Файл не найден'),404
    try:
        p=share_path(f['share_token'],f['stored_path'])
        p.unlink(missing_ok=True)
    except Exception:
        pass
    c=db()
    c.execute('DELETE FROM received_files WHERE id=?',(fid,))
    c.commit()
    c.close()
    return jsonify(ok=True)
...
def download_received(fid):
    ...
    if not f:
        return jsonify(error='Файл не найден'),404
    if u['role']!='admin' and f['recipient_id']!=u['id']:
...
def revoke_local_share(token):
    ...
    if r['owner_id']!=user()['id']:
        return jsonify(error='Только владелец'),403
    c=db()
    c.execute('UPDATE local_shares SET active=0 WHERE token=?',(token,))
    c.commit()
    c.close()
    return jsonify(ok=True)
...
def project_env(pid):
    ...
    env.update({
        ...
        'PYSPACE_PROJECT_ID':str(pid),
        'PYTHONPATH':str(packages)+os.pathsep+env.get('PYTHONPATH','')
    })
    return work,env
    ...
    try:
        r=subprocess.run(
            [sys.executable,'-m','pip','install','--disable-pip-version-check',
             '--no-input','--no-cache-dir','--target',str(target),package],
        ...
        return jsonify(ok=False,returncode=-1,output='⏱ pip install превысил лимит 60 секунд.'),504
...
def parse_terminal(command):
    ...
    if not all(TERMINAL_RE.fullmatch(a) for a in args):
        raise ValueError('Недопустимый аргумент')
    if args[0] in ('python','python3'):
        if '-c' in args:
            raise ValueError('python -c отключён в терминале')
        if '-m' in args:
            if len(args)<3 or args[2] not in ('pip','pip3'):
                raise ValueError('Разрешён только python -m pip')
        else:
            allowed_flags={'--version','-V'}
            if any(a.startswith('-') and a not in allowed_flags for a in args[1:]):
                raise ValueError('Флаг Python запрещён')
            for a in args[1:]:
                if not a.startswith('-') and (a.startswith('/') or a=='..' or a.startswith('../') or '/..' in a):
                    raise ValueError('Доступ за пределы проекта запрещён')
    if args[0]=='pip':
        if len(args)>1 and args[1] not in ('list','freeze','show','check','--version','-V'):
            raise ValueError('Разрешены только pip list, freeze, show, check и --version')
...
def terminal():
    ...
    if not access(pid, True):
        return jsonify(ok=False,error='Нет прав'),403
    if not command:
        return jsonify(ok=False,error='Введите команду'),400
    if len(command)>4000:
        return jsonify(ok=False,error='Команда слишком длинная'),413
    try:
        args=parse_terminal(command)
    except ValueError as e:
...
def run():
    ...
    pid=int(d.get('project_id') or 0)
    try:
        path=clean(d.get('path') or 'main.py')
    except ValueError as e:
        return jsonify(error=str(e)),400
    if not access(pid):
        return jsonify(error='Нет доступа'),403
    try:
        target=fpath(pid,path)
    except ValueError as e:
        return jsonify(error=str(e)),400
    ...
    if ext=='.css':
        return jsonify(ok=True,kind='css',output='CSS готов к предпросмотру')
    if ext=='.sql':
        with tempfile.TemporaryDirectory(prefix='pyspace_sql_') as td:
            dbfile=Path(td)/'project.sqlite3'
            ...
            try:
                ...
                for t in cur.execute("SELECT name FROM sqlite_master WHERE type='table' AND name NOT LIKE 'sqlite_%' LIMIT 20").fetchall():
                    try:
                        rows.extend([dict(r) for r in con.execute(f'SELECT * FROM "{t[0].replace(chr(34),chr(34)*2)}" LIMIT 100').fetchall()])
                    except Exception:
                        pass
                return jsonify(ok=True,kind='sql',output='SQL выполнен успешно\n'+('\n'.join(str(r) for r in rows) if rows else 'Изменения применены.'))
            except Exception as e:
                return jsonify(ok=False,kind='sql',output=str(e))
            finally:
                con.close()
    if ext not in ('.py','.pyw'):
        return jsonify(error='Для запуска поддерживаются Python, HTML, CSS и SQL'),400
    stdin_data=str(d.get('stdin',''))
    if len(stdin_data)>20000:
        return jsonify(error='Тестовые данные слишком большие'),413
    with tempfile.TemporaryDirectory(prefix='pyspace_') as td:
        work=Path(td)/'project'
        shutil.copytree(pdir(pid),work,ignore=shutil.ignore_patterns('.packages','__pycache__','*.pyc','.upload.zip'))
        script=work/path
        ...
        try:
            ...
            return jsonify(ok=False,returncode=-1,output=f'⏱ Превышен лимит {TIMEOUT} сек.\nВозможна бесконечная input()/петля.'),504
...
def users():
    c=db()
    r=c.execute('SELECT id,username,role,created_at FROM users ORDER BY id').fetchall()
    c.close()
...
def role(uid):
    ...
    c=db()
    c.execute('UPDATE users SET role=? WHERE id=?',(r,uid))
    c.commit()
    c.close()
    return jsonify(ok=True)
