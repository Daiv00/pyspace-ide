...
│  ├─ keepalive.py         самопинг публичного адреса против засыпания
...
| `PYSPACE_BACKUP_PATH` | `backups/pyspace-data.tar.gz` | путь файла копии |
| `PYSPACE_BACKUP_INTERVAL` | `900` | период автосохранения, с (минимум 300) |
| `PYSPACE_BACKUP_MAX_MB` | `40` | предохранитель по размеру архива |
| `PYSPACE_BACKUP_RESTORE` | `1` | `0` — не восстанавливать автоматически при старте |
...
⚠️ На плане Free диск не подключается: без автобэкапа файлы проектов и база живут
только до следующего деплоя или перезапуска. Два решения — автобэкап в приватный
репозиторий GitHub (бесплатно, раздел 6 в DEPLOY.md) или платный диск (раздел 4).
